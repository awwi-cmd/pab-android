...
D-090. **Pickup SFX (gems/potions/chests), the HP/XP bars redesigned as
one stacked red/yellow HUD at the top of the screen (full width, the
pause/debug-die buttons padded below it so nothing overlaps), and a new
20-achievement ACHIEVEMENTS screen (between START and SETTINGS) that
grants coin/gem rewards to the wallet automatically as lifetime stats
cross thresholds are built, on-device verification pending** — see
D-091. This is real, ongoing
post-demo work
...
Kept current as of Phase 37 (2026-09-12) — update this tree when you add a
...
└── arena/                    <- the Flutter project
    ...
    └── lib/
        ...
        ├── core/
        │   ├── constants.dart     // design size, colors (ArenaColors.warning is the warm-amber exclusive-skill accent, D-083; ArenaColors.xp is the XP bar's yellow, D-091), layer priorities, render scales, kHudBar* shared HP/XP bar layout consts — D-091
        ...
        │   ├── bgm_controller.dart // app-wide singleton: single base-layer BGM — D-062, layering reverted by D-064, track swapped to electric-eel-fishing.ogg by D-073, FlameAudio.loopLongAudio (MediaPlayer, static-free) by D-075; pause()/resume() on app background/foreground (WidgetsBindingObserver in app.dart) — D-086; startup-race with pause/resume fixed via _pausedByLifecycle — D-087
        ...
        │   ├── sfx_player.dart    // app-wide singleton: every one-shot SFX (footsteps, damage, tap, projectile-shoot, level-up, chest-card select/chosen, explosion, death, pickup) — D-076/D-085/D-091; pooled via AudioPool (fixed a real resource leak) — D-079; PlayerMode.lowLatency + drop-not-queue guards (fixed a real crash, root-caused via logcat) — D-081; AudioPool now passed FlameAudio.audioCache explicitly (fixed total pooled-SFX silence, root-caused via reading audioplayers' own source) — D-088
        │   ├── meta_progression.dart // MetaStat (STR/VIT/DEX/INT/CORRUPTION [displayed "CHAOS", D-071]/HASTE/FORTUNE/RESOLVE/MAGNET/LUCK/REGEN/CRIT), coins+gems+lifetimeKills+ownedItemIds wallet; debugAdjustCoins/debugAdjustGems/debugResetWallet — D-047/D-055/D-059/D-067/D-069; 6 more lifetime counters + claimedAchievementIds, recordRoundEnd (one atomic round-over write incl. achievement grants) — D-091
        │   ├── achievements.dart  // Achievement/kAchievements (20 entries)/buildStatValues/isAchievementMet -- zero imports of meta_progression.dart on purpose (avoids a 2-file cycle) — D-091
        ...
        ├── ui/
        │   ├── screens/
        │   │   ├── main_menu_screen.dart      // Stateful: auto-pushes TutorialScreen on a fresh save, splash_bg.png as full-bleed background (title art baked in, own Text('ARENA') removed), circular "?" button — D-082/D-084; ACHIEVEMENTS button between START/SETTINGS — D-091
        │   │   ├── achievements_screen.dart    // scrollable read-only list of kAchievements w/ progress bars, dimmed until claimed — no claim button, rewards auto-grant at round-end — D-091
        ...
        └── game/
            ...
            │   ├── chest.dart                // world chest: anima+explosion then chest_01-12 opening sequence — D-055; pickup radius widened by MAGNET — D-069; anima sized off the chest's own render size + reduced contrast — D-071; anima renders behind the chest — D-074; pickup SFX on touch, distinct from the explosion beat — D-091
            │   ├── chest_spawner.dart        // periodic random-area drop, same shape as potion_spawner.dart — D-055
            │   ├── hp_bar.dart               // now on camera.viewport (HUD), not world — D-040; top of screen, full width, red fill — D-091
            │   ├── xp_bar.dart               // twin of hp_bar.dart, positioned via onGameResize — D-076; stacked directly under HP (was bottom-anchored), yellow fill — D-091
            │   ├── damage_text.dart
